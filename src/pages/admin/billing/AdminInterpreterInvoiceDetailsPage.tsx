  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white shadow-sm border border-gray-200 rounded-xl p-6">
        ...
        <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-4 flex items-center px-6">
          <FileText size={14} className="mr-2" /> Line Items Breakdown
        </h3>
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-slate-50">
              <tr>
                <th className="px-6 py-4 text-left text-[10px] font-black tracking-widest text-slate-400 uppercase">Category</th>
                <th className="px-6 py-4 text-left text-[10px] font-black tracking-widest text-slate-400 uppercase">Description</th>
                <th className="px-6 py-4 text-right text-[10px] font-black tracking-widest text-slate-400 uppercase">Total</th>
              </tr>
            </thead>
              ...
              {invoice.items?.map((item: any, idx: number) => (
                <tr key={idx} className="hover:bg-slate-50 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="px-2 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-black uppercase tracking-wider rounded border border-indigo-100">
                      {item.category?.replace(/_/g, ' ') || 'PAYMENT'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-slate-700 font-medium">{item.description}</td>
                  <td className="px-6 py-4 text-sm font-black text-slate-900 text-right">£{item.total.toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="p-8 bg-slate-50 border-t border-slate-200 text-right">
          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Total Payout</p>
          <p className="text-3xl font-black text-slate-900">£{invoice.totalAmount.toFixed(2)}</p>
        </div>
      </div>
    </div>
  );
